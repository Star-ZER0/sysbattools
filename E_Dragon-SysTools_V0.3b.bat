...
echo 　　　　　　　　██　██　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　
echo ————————————————————————菜单————————————————————————
echo [1]清理系统空间			[11]开启桌面快捷方式的小箭头	[21]定时执行指定命令
echo [2]关闭Windows Defender		[12]执行chkdsk并尝试修复磁盘	[22]去除桌面右下角激活水印
echo [3]开启Windows Defender		[13]修复指定分区引导		[23]开启桌面右下角激活水印
echo [4]关闭休眠(可释放部分空间)	[14]FAT32无损转NTFS		[24]禁用索引服务
echo [5]关闭客户体验改善计划		[15]扫描系统文件的完整性并修复	[25]开启索引服务
echo [6]关闭磁盘碎片整理计划		[16]扫描指定文件的完整性并修复	[26]开启快速启动
echo [7]设置用户账户控制(UAC)弹窗	[17]编辑hosts文件		[27]关闭快速启动
echo [8]关闭用户账户控制(UAC)小盾牌	[18]刷新DNS缓存			[28]配置超级预读服务
echo [9]开启用户账户控制(UAC)小盾牌	[19]定时关机			[29]配置自动登录及登录选项
echo [10]关闭桌面快捷方式的小箭头	[20]定时重启			
...
:sfcfile
cls
echo 本工具使用Windows自带的sfc工具扫描一次您当前输入路径的系统文件，并尝试修复错误
echo 适合电脑被病毒侵扰后系统文件损坏导致的系统不稳定的修复
echo 请输入想要修复的文件完整路径：(如：C:\Windows\explorer.exe)
set /p sfcfilepath=
echo 您将要修复"%sfcfilepath%"是否要继续执行？[1]继续	[2]返回：
set /p sfcfile=
